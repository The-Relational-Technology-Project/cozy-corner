            ...
            <div className="bg-white/60 backdrop-blur-sm rounded-2xl p-8 shadow-lg border border-white/20">
              ...
              <Collapsible open={isIdeaFormOpen} onOpenChange={setIsIdeaFormOpen}>
                <CollapsibleTrigger asChild>
                  <Button 
                    variant="outline" 
                    className="mb-4 border-amber-300 text-amber-700 hover:bg-amber-50 rounded-xl bg-white/80"
                  >
                  ...
                  <Card className="bg-white/80 backdrop-blur-sm border-amber-200">
                    <CardContent className="p-6">
                      <form onSubmit={handleIdeaSubmit} className="space-y-4">
                        ...
                        <Button 
                          type="submit" 
                          className="w-full bg-gradient-to-r from-amber-600 to-orange-600 hover:from-amber-700 hover:to-orange-700 text-white font-medium py-3 rounded-xl"
                        >
                          Submit Idea
                        </Button>
                      </form>
                  ...
                  {category.items.map((role, roleIndex) => (
                    <Dialog key={roleIndex} open={isDialogOpen && selectedRole === role} onOpenChange={(open) => {
                      ...
                      <DialogTrigger asChild>
                        <Card 
                          className={`cursor-pointer transition-all duration-200 hover:shadow-lg hover:scale-105 border-amber-200 ${
                            role.isSpecial ? 'border-orange-500 bg-gradient-to-br from-orange-50 to-amber-50' : 'hover:border-amber-400'
                          }`}
